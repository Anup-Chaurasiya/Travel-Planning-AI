		...
		<Card className="border-y-2 p-5">
			...
			<div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 xl:grid-cols-3 text-justify gap-3 md:gap-6 xl:gap-6 mt-4">
				{trip?.tripData?.hotels?.map((hotel, index) => (
					<div
						key={index}
						className="text-sm lg:text-base hover:scale-105 transition-all mb-2 border-[1px] md:border-2 border-customGreen dark:border-customBlue rounded-lg px-2 cursor-pointer"
					>
						<Link
							to={
